...
function VideoCard({ item }) {
  ...
  return (
    <div className="group relative border border-white/[0.06] rounded-2xl overflow-hidden bg-deep/30 hover:border-amber/25 transition-all duration-500 hover:-translate-y-1.5">
          ...
          <div className="absolute inset-0 vhs-grad">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-14 h-14 md:w-16 md:h-16 rounded-full border-2 border-white/20 flex items-center justify-center group-hover:scale-110 group-hover:border-amber/50 transition-all duration-500">
                <svg
                  className="w-4 h-4 md:w-5 md:h-5 text-white/50 ml-0.5"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
            ...
            <div className="absolute top-3 left-4 text-[8px] tracking-[0.2em] text-white/35 font-mono">
              {item.no}
            </div>
            ...
            <div className="absolute bottom-3 left-4 text-[8px] tracking-[0.15em] text-white/15 font-mono">
              00:00:00:00
            </div>
            <div className="absolute -bottom-3 -right-3 text-[6rem] md:text-[8rem] leading-none font-display text-white/[0.02] select-none">
              {item.no.split(' ').pop()}
            </div>
          </div>
        )}
      ...
      <div className="p-5">
        <p className="text-[9px] tracking-[0.22em] text-teal font-mono">
          {item.tag}
        </p>
        <h3 className="mt-2 font-song text-xl font-bold group-hover:text-amber transition-colors">
          {item.t}
        </h3>
        <p className="mt-2 text-sm text-ink/48 leading-relaxed">{item.d}</p>
        <div className="mt-4 flex justify-between items-center">
          <span className="text-[9px] font-mono text-ink/25">
            <span className="text-amber/60 mr-1">&#9654;</span>
            {item.dur}
          </span>
          <span className="text-[9px] font-mono text-ink/15">JAZZ OST</span>
        </div>
      </div>
    </div>
  )
